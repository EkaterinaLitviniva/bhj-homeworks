...
const robotReply = () => {
  ...
  chatMessages.innerHTML += 
    `<div class="message"><div class="message__time">${new Date().toLocaleTimeString().substr(0,5)}</div>
    <div class="message__text">${messages[index]}</div></div>`;
...
inputMessage.addEventListener('keydown', function(e) {
  ...
  if (this.value.length === 0)
    return;
  chatMessages.innerHTML += 
    `<div class="message message_client"><div class="message__time">${new Date().toLocaleTimeString().substr(0,5)}</div>
    <div class="message__text">${this.value}</div></div>`;  
  this.value = '';
  robotReply();
});